...
- `codegen-<lang>/` — one directory per supported target language; each holds the `.gracerules*` triad, pattern templates, and language-specific type and quality references
- `references/` — per-connector tech specs generated by `grace techspec` (gitignored)
