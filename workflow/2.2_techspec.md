...
cat ../{connector_name}.txt | grace techspec {Connector_Name} --target-lang {LANG} -e
```
...
```bash
# From connector-service root:
# Lang-agnostic: search all rulesbook references regardless of language.
# This works for both `codegen-rust/references/` and `codegen-python/references/`.
find grace/rulesbook -iname "*{connector}*" -path "*/references/*" | head -20
```
