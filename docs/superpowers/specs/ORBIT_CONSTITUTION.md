...
**Status**: v0.9 — mandate plan upgrade/downgrade: `MandateConnector` / `MandatesFacade` gain `create_plan` + `change_plan` (Cashfree Plans + `CHANGE_PLAN`), with `CreatePlanRequest`/`CreatePlanResponse`/`ChangePlanRequest` (lens 0.4.0). Customer-chosen mandate rail: `CreateSubscriptionRequest.rail` → `rails: list[MandateRail] | None`, and `MandateWebhookEvent` / `SyncSubscriptionResponse` surface the realized rail (lens 0.3.0). `CreateSubscriptionRequest` exposes optional Cashfree authorization-amount fields (lens 0.2.1). Connector version gate removed (connectors ship bundled with lens); periodic subscription-mandate support (Phase 3); capability-interface model; shared WebhookRouter.
**Date**: 2026-06-03.
...
- Per-flow request/response models (mandates): `CreateSubscriptionRequest`/`CreateSubscriptionResponse`, `SyncSubscriptionRequest`/`SyncSubscriptionResponse`, `ManageMandateRequest`/`ManageMandateResponse`, and (v0.9 plan management) `CreatePlanRequest`/`CreatePlanResponse` + `ChangePlanRequest` (whose response is `ManageMandateResponse`). Common base: `MandateRequestCommon` (fields: `merchant_id` only — no `order_id`; mandates are not orders). Supporting types: `CustomerContact` (email + phone), `ApprovalHandle` (type/url/session_id/raw).
...
- `MandateWebhookEvent` — the parsed, normalized form of an inbound PSP mandate/subscription event; carries `mandate_status`, `debit: MandateDebitOutcome | None`, `psp_mandate_ref`, and — on a successful authorization — the realized debit instrument the customer chose: `realized_rail: MandateRail | None`, `authorization_reference: str | None`, and the raw `payment_group: str | None`. The same three realized-rail fields appear on `SyncSubscriptionResponse` (so Orbit's reconcile backstop recovers the rail when the auth webhook is missed). `CreateSubscriptionRequest` carries `rails: list[MandateRail] | None` (None/empty = offer every supported instrument), letting the customer choose the rail on the PSP authorization screen rather than the caller pinning one at create.
...
- **2026-06-01 v0.7** — Additive: `CreateSubscriptionRequest` gains `authorization_amount` (optional `Amount`) and `authorization_amount_refund` (`bool`, default `True`), exposing Cashfree's subscription verification charge. Shipped in **lens 0.2.1** with the Cashfree sandbox-bug fixes (the connector hardcoded `authorization_amount=0`, which Cashfree rejected with `auth_amount_invalid_for_action`; also fixed: order-sync now keys on the merchant `order_id`, and `_map_http_error` surfaces `psp_code`/`psp_message`). Both new fields are optional + defaulted (backward-compatible), so it ships as a **patch (0.2.1)** alongside the bugfix rather than a standalone minor; no migration. Coordinated updates: `SUBPROJECT_LENS.md` §4.4 (the two fields) + the lens public-surface snapshot; the Grace-generated Cashfree connector regenerated (grace 0.8.0) to consume them.
- **2026-06-03 v0.8** — Customer-chosen mandate rail (breaking create-side + additive read-side). Orbit stops pinning a single debit instrument at subscription create: `CreateSubscriptionRequest.rail: MandateRail` (required) becomes `rails: list[MandateRail] | None` (`None`/empty = offer every instrument the connector supports — the customer picks UPI Autopay vs card/e-mandate on the PSP authorization screen; a subset constrains the choice). lens surfaces the *realized* instrument back: `MandateWebhookEvent` and `SyncSubscriptionResponse` each gain `realized_rail: MandateRail | None`, `authorization_reference: str | None` (UMN/UMRN/enrollment id), and the raw `payment_group: str | None` — populated only on a successful authorization, on **both** the auth webhook and `sync_subscription` (so Orbit's reconcile backstop recovers the rail when the auth webhook is dropped). **Breaking** (field rename / signature change on a locked model — §8 "Rename / signature change") → lens **0.3.0** (in lens's 0.x line the major-position bump lands on the minor digit, as with the v0.5 reshape → 0.2.0). Migration: Orbit replaces `rail=X` with `rails=[X]` (or `None`/omit to offer all); no persisted-data migration (no production Orbit consumer yet — clean break). Coordinated updates: §5 (`MandateWebhookEvent`/`SyncSubscriptionResponse` realized-rail fields, `CreateSubscriptionRequest.rails`) + this changelog (§8); `SUBPROJECT_LENS.md` §4.4 (the three models) + the lens public-surface snapshot (`tests/snapshot/public_surface.py`); `SUBPROJECT_ORBIT_PRODUCT.md` (the Orbit↔Lens boundary — §5 requires both boundary specs revised: `POST /v1/mandates` `rails` body, the `mandates`-table `requested_rails`/`realized_rail`/`authorization_reference` columns, and the §9.2 consumed-surface + §9.3 reconcile-backstop notes); lens `connector.py` / `webhooks.py` / `status_map.py` hand-edited as the Grace reference — the Cashfree Grace rulebook must reproduce them in the next regeneration wave (regeneration prompt: `2026-06-03-grace-cashfree-rails-regen-prompt.md`; Cashfree stays quarantined until then). No Grace ABC change (the `Connector` capability interfaces are untouched), so no connector-regeneration mandate beyond Cashfree itself.
- **2026-06-03 v0.9** — Mandate plan upgrade/downgrade. `MandateConnector` and `MandatesFacade` gain two async plan-management flows: `create_plan(CreatePlanRequest) -> CreatePlanResponse` (create a reusable PSP PERIODIC plan → `plan_id`) and `change_plan(ChangePlanRequest) -> ManageMandateResponse` (move an active periodic subscription onto a new plan — the downgrade path and the lower-friction all-rail upgrade alternative; the in-place upgrade stays orbit-side via `CreateSubscriptionRequest.authorization_amount` re-auth, no lens change). New domain types: `CreatePlanRequest`, `CreatePlanResponse`, `ChangePlanRequest`. On Cashfree these map to the Plans Create API (`POST /plans`) and the `CHANGE_PLAN` manage action; Cashfree enforces "new recurring amount ≤ the authorized `plan_max_amount`" and is PERIODIC-only, surfaced as `ConnectorError(INVALID_REQUEST)` with `psp_code`/`psp_message`. **Breaking** (two new abstract methods on the locked `MandateConnector` ABC — every implementer must add them; §8 "signature change") → lens **0.4.0**. Out of scope (deferred to a follow-up): on-demand subscription create mode + the Charge Mandate / controlled-execution API for usage-based billing (on-demand + UPI-only). Migration: no persisted-data migration (no production Orbit consumer yet); the bundled Cashfree connector + the test stub were updated in-tree. Coordinated updates: §5 (`MandateConnector`/`MandatesFacade` method lists + the three new mandate models) + §8 locked set (the two plan-management flows) + this changelog; `SUBPROJECT_LENS.md` §4.1/§4.2/§4.4 + the lens public-surface snapshot (`tests/snapshot/public_surface.py`); `SUBPROJECT_ORBIT_PRODUCT.md` §9 (the Orbit↔Lens boundary — Orbit's upgrade/downgrade consumption of `create_plan`/`change_plan`). `SUBPROJECT_GRACE_CODEGEN.md` (Status → v0.7; §3.2 layout method count; §5 rubric) — because this changes the `MandateConnector` ABC, §8's generated-connector regeneration policy applies (a Grace bump targeting the new ABC). lens `connector.py` / `models.py` hand-edited as the Grace reference; to be reproduced in the combined Cashfree regen alongside the 0.3.0 rails work (regen prompt: `2026-06-03-grace-cashfree-rails-regen-prompt.md`). Only Cashfree is committed, so only it regenerates.
