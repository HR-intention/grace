...
description: Use when the user wants to add a new flow pattern (e.g., pattern_<flow>.md) or payment-method pattern under `rulesbook/codegen-rust/guides/patterns/`. Walks through the required section shape, prerequisite chain, macro template usage, and wiring into the right `.gracerules*` entrypoint so the new pattern is actually applied by codegen.
---

# Add a connector pattern to the rulesbook

Patterns under `rulesbook/codegen-rust/guides/patterns/` are templates the external AI coding agent applies when implementing Rust connectors. A pattern that's structurally off — or not wired into `.gracerules*` — will silently never be used.
...
|---|---|---|
| **Core flow** (Authorize/Capture/Refund/Void/PSync/RSync) | `rulesbook/codegen-rust/guides/patterns/pattern_<flow>.md` | `.gracerules` (always) |
| **Advanced flow** (SetupMandate, RepeatPayment, IncomingWebhook, DefendDispute, etc.) | `rulesbook/codegen-rust/guides/patterns/pattern_<flow>.md` | `.gracerules_add_flow` (and `.gracerules` if listed in core 6) |
| **Payment method (under Authorize)** | `rulesbook/codegen-rust/guides/patterns/authorize/<category>/pattern_authorize_<pm>.md` | `.gracerules_add_payment_method` |
...
- Flow pattern: [pattern_authorize.md](rulesbook/codegen-rust/guides/patterns/pattern_authorize.md), [pattern_refund.md](rulesbook/codegen-rust/guides/patterns/pattern_refund.md)
- Payment-method pattern: [authorize/upi/pattern_authorize_upi.md](rulesbook/codegen-rust/guides/patterns/authorize/upi/pattern_authorize_upi.md), [authorize/crypto/pattern_authorize_crypto.md](rulesbook/codegen-rust/guides/patterns/authorize/crypto/pattern_authorize_crypto.md)
...
3. **🏗️ Modern Macro-Based Template** — Rust code block using `macro_connector_implementation!` with the right arguments. Cross-check the macro signature against [macro_patterns_reference.md](rulesbook/codegen-rust/guides/patterns/macro_patterns_reference.md).
...
These come from the UCS architecture decision in [rulesbook/codegen-rust/README.md](rulesbook/codegen-rust/README.md). Any pattern violating them produces broken connectors.
...
grep -n "pattern_" rulesbook/codegen-rust/.gracerules | head -20
grep -n "pattern_" rulesbook/codegen-rust/.gracerules_add_flow | head -20
```
