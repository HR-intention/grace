...
`ConnectorErrorReason` values (the only ones that exist — these are for **raised exceptions**, separate from `PaymentFailureCode` which is for **failed attempts**):

`INVALID_REQUEST`, `AUTHENTICATION_FAILED`, `AUTHORIZATION_FAILED`, `RATE_LIMITED`, `PSP_UNAVAILABLE`, `PSP_TIMEOUT`, `ORDER_NOT_FOUND`, `PAYMENT_NOT_FOUND`, `REFUND_NOT_FOUND`, `INVALID_ORDER_STATE`, `WEBHOOK_SIGNATURE_FAILED`, `NOT_SUPPORTED`, `INCOMPATIBLE_VERSION`, `INTERNAL`.

| ✗ Invented `ConnectorErrorReason` | ✓ Locked equivalent |
|---|---|
| `ConnectorErrorReason.PSP_ERROR` | `ConnectorErrorReason.INTERNAL` (catch-all 4xx) or `PSP_UNAVAILABLE` (5xx). `PSP_ERROR` is on `PaymentFailureCode`, not this enum. |
| `ConnectorErrorReason.UNKNOWN` | `ConnectorErrorReason.INTERNAL`. (`UNKNOWN` exists on `PaymentFailureCode` only.) |
| `ConnectorErrorReason.GENERIC_ERROR` / `SERVER_ERROR` | `INTERNAL` or `PSP_UNAVAILABLE` |
| `ConnectorErrorReason.TIMEOUT` | `PSP_TIMEOUT` |
| `ConnectorErrorReason.INVALID_PARAMETER` | `INVALID_REQUEST` |

Common confusion: `PaymentFailureCode` and `ConnectorErrorReason` have overlapping-looking-but-distinct vocabularies. `PSP_ERROR` IS valid on `PaymentFailureCode` (a failed PaymentAttempt) but NOT on `ConnectorErrorReason` (a raised exception). Check the enum, not the vibe.
